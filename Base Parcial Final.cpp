...
////////////////////////////////////////////////////////
void reporteProductosStockBajo(Producto p[50]);
void reporteCantidadComentarios(Comentario c[50]);
void reportePrecioMAX_MIN(Producto p[50]);
...
int main() {
    ...
    if (iniciarSesion(usuarios, indiceUsuario) == 1) {
        ...
        do {
            ...
            cout << "\t9. Reportes adicionales" << endl;
            cout << "\t10. Salir" << endl;
            cout << "\n\tOpcion: "; cin >> opc;
            ...
            switch (opc) {
                ...
                case 9:
                	...
                	do{
                		...
                		cout << "\n -----  REPORTES ADICIONALES  -----" << endl;
                		cout << "\n   1. Productos con stock bajo" << endl;
                		cout << "   2. Cantidad de comentarios por fecha" << endl;
                		cout << "   3. Precio máximo y mínimo de productos" << endl;
                		cout << "   4. Salir" << endl;
                		cout << "\n   Opcion: "; cin >> opc2;
                		...
                		switch(opc2)
                		{
                			case 1: 
                				system("pause"); system("cls");
                				reporteProductosStockBajo(productos);
                				break;
                				
                			case 2: 
                				system("pause"); system("cls");
                				reporteCantidadComentarios(comentarios);
                				break;
                				
                			case 3:
                				system("pause"); system("cls");
                				reportePrecioMAX_MIN(productos);
                				break;
...
void reporteProductosStockBajo(Producto p[50])
{
	...
	ofstream archivo("reporte_stock_bajo.txt");
    ...
    archivo << "ID,Nombre,Stock" << endl;

    cout<<"\n----- 5 PRODUCTOS CON MENOR STOCK -----\n"<<endl;

    for(int i=0; i<5; i++)
    {
        cout<<"ID: "<<aux[i].idProducto
            <<" | Nombre: "<<aux[i].nombre
            <<" | Stock: "<<aux[i].stock
            <<endl;
            
        archivo << aux[i].idProducto << ","
                << aux[i].nombre << ","
                << aux[i].stock
                << endl;
    }
...
void reporteCantidadComentarios(Comentario c[50])
{
    ...
    cout << "Fecha: " << fechaBuscar << endl;
    cout << "Cantidad de comentarios: " << cantidad << endl;
    
    ofstream archivo("reporte_comentarios.txt");

	archivo << "REPORTE DE COMENTARIOS\n\n";	
	archivo << "Fecha: " << fechaBuscar << endl;
	archivo << "Cantidad de comentarios: " << cantidad << endl;
...
void reportePrecioMAX_MIN(Producto p[50])
{
    double precioMax = p[0].precio;
    double precioMin = p[0].precio;

    string productoMax = p[0].nombre;
    string productoMin = p[0].nombre;

    for(int i = 1; i < 50; i++)
    {
        if(p[i].precio > precioMax)
        {
            precioMax = p[i].precio;
            productoMax = p[i].nombre;
        }

        if(p[i].precio < precioMin)
        {
            precioMin = p[i].precio;
            productoMin = p[i].nombre;
        }
    }

    cout << "\n----- REPORTE PRECIO MAXIMO Y MINIMO -----\n" << endl;

    cout << "Producto mas costoso: "
         << productoMax
         << " - $" << precioMax << endl;

    cout << "Producto mas economico: "
         << productoMin
         << " - $" << precioMin << endl;
         
    ofstream archivo("reporte_precios.txt");
    
    archivo << "REPORTE PRECIO MAXIMO Y MINIMO\n\n";

	archivo << "Producto mas costoso: "
	        << productoMax
	        << " - $" << precioMax << endl;
	
	archivo << "Producto mas economico: "
	        << productoMin
	        << " - $" << precioMin << endl;
